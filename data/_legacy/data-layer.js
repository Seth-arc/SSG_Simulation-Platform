...
// ==========================================
// 9.5 SESSION VALIDATION & DEBUGGING
// ==========================================

/**
 * Validate that the current session is properly set up
...
function validateSession() {
    ...
    if (!CURRENT_SESSION_ID) {
        const storedSessionId = sessionStorage.getItem('esg_session_id');
        if (storedSessionId) {
            // Session ID exists in storage but not in memory - sync it
            CURRENT_SESSION_ID = storedSessionId;
            result.sessionId = storedSessionId;
            result.warnings.push('Session ID was in sessionStorage but not in memory - synced');
        } else {
            result.valid = false;
            result.issues.push('No session ID found - user must join a session first');
        }
    }
    ...
    if (!CURRENT_CLIENT_ID) {
        result.valid = false;
        result.issues.push('No client ID - this should never happen');
    }

    // Check role
    if (!result.role) {
        result.warnings.push('No role set - some features may not work correctly');
    }

    // Log validation result
    if (result.issues.length > 0) {
        console.error('[SESSION VALIDATION] Issues found:', result.issues);
    }
    if (result.warnings.length > 0) {
        console.warn('[SESSION VALIDATION] Warnings:', result.warnings);
    }
    if (result.valid) {
        console.log('[SESSION VALIDATION] Session is valid:', result.sessionId);
    }
...
/**
 * Ensure session ID is synchronized between memory and storage
 * Call this at the start of any data operation
 */
function ensureSessionSync() {
    const storedSessionId = sessionStorage.getItem('esg_session_id');

    if (storedSessionId && !CURRENT_SESSION_ID) {
        console.log('[SESSION SYNC] Syncing session ID from storage:', storedSessionId);
        CURRENT_SESSION_ID = storedSessionId;
    } else if (CURRENT_SESSION_ID && !storedSessionId) {
        console.log('[SESSION SYNC] Storing session ID to storage:', CURRENT_SESSION_ID);
        sessionStorage.setItem('esg_session_id', CURRENT_SESSION_ID);
    } else if (CURRENT_SESSION_ID !== storedSessionId && storedSessionId) {
        // Prefer the stored one if they differ (user may have switched tabs)
        console.warn('[SESSION SYNC] Session ID mismatch - using stored ID:', storedSessionId);
        CURRENT_SESSION_ID = storedSessionId;
    }
...
/**
 * Get consistent storage key for actions
 * @param {string} sessionId - Session ID
 * @param {number} move - Move number
 * @returns {string} Storage key
 */
function getActionsStorageKey(sessionId, move) {
    if (sessionId) {
        return `blueActions_session_${sessionId}_move_${move}`;
    }
    return `blueActions_move_${move}`;
}

/**
 * Get consistent storage key for requests
 * @param {string} sessionId - Session ID
 * @param {number} move - Move number
 * @returns {string} Storage key
 */
function getRequestsStorageKey(sessionId, move) {
    if (sessionId) {
        return `blueRequests_session_${sessionId}_move_${move}`;
    }
    return `blueRequests_move_${move}`;
}

/**
 * Get consistent storage key for communications
 * @param {string} sessionId - Session ID
 * @param {number} move - Move number
 * @returns {string} Storage key
 */
function getCommunicationsStorageKey(sessionId, move) {
    if (sessionId) {
...
/**
 * Get consistent storage key for white cell data
 * @param {string} sessionId - Session ID
 * @param {number} move - Move number
 * @returns {string} Storage key
 */
function getWhiteCellStorageKey(sessionId, move) {
    if (sessionId) {
        return `whiteCell_session_${sessionId}_move_${move}`;
    }
    return `whiteCell_move_${move}`;
}

/**
 * Get data from storage, trying session-based key first, then legacy
 * @param {string} dataType - Type of data (actions, requests, etc.)
 * @param {number} move - Move number
 * @param {*} defaultValue - Default value if not found
 * @returns {Object} Data and the key it was found in
 */
function getDataWithFallback(dataType, move, defaultValue = null) {
    ensureSessionSync();
    const sessionId = CURRENT_SESSION_ID;

    const keyGenerators = {
        actions: [
            () => sessionId ? `blueActions_session_${sessionId}_move_${move}` : null,
            () => `blueActions_move_${move}`,
            () => `actions_move_${move}`,
            () => `blueActionsSubmittedMove${move}`
        ],
        requests: [
            () => sessionId ? `blueRequests_session_${sessionId}_move_${move}` : null,
            () => `blueRequests_move_${move}`,
            () => `blueRequestsSubmittedMove${move}`
        ],
        communications: [
            () => sessionId ? `communications_session_${sessionId}_move_${move}` : null,
            () => `communications_move_${move}`
        ],
        whiteCell: [
            () => sessionId ? `whiteCell_session_${sessionId}_move_${move}` : null,
            () => `whiteCell_move_${move}`
        ],
        notes: [
            () => sessionId ? `notes_session_${sessionId}_move_${move}` : null,
            () => `notes_move_${move}`,
            () => `notetaker_move_${move}`
        ]
    };

    const generators = keyGenerators[dataType] || [];
    ...
    for (const generator of generators) {
        ...
        if (key) {
            const data = safeGetItem(key, null);
            if (data !== null) {
                console.log(`[DATA LAYER DEBUG] Found ${dataType} data in key: ${key}`);
                return { data, key, found: true };
            }
        }
    }

    console.log(`[DATA LAYER DEBUG] No ${dataType} data found for move ${move}`);
    return { data: defaultValue, key: null, found: false };
}

/**
 * Display session status banner (for debugging)
 */
function showSessionStatus() {
    const validation = validateSession();
    const statusDiv = document.createElement('div');
    statusDiv.id = 'esg-session-status';
    statusDiv.style.cssText = `
        ...
        right: 0;
        padding: 8px 16px;
        background: ${validation.valid ? '#10b981' : '#ef4444'};
        color: white;
    ...
    statusDiv.innerHTML = `
        <span>
            <strong>Session:</strong> ${validation.sessionId || 'NOT SET'} |
            <strong>Role:</strong> ${validation.role || 'NOT SET'} |
            <strong>Client:</strong> ${validation.clientId?.substring(0, 8) || 'NOT SET'}...
        </span>
    ...
    const existing = document.getElementById('esg-session-status');
...
window.esg = {
    ...
    // Session Validation & Debugging (NEW)
    validateSession,
    ensureSessionSync,
    showSessionStatus,
    getDataWithFallback,

    // Storage Key Helpers (NEW)
    getActionsStorageKey,
    getRequestsStorageKey,
    getCommunicationsStorageKey,
    getWhiteCellStorageKey
};
