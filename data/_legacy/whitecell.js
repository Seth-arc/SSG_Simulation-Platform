...
// FIXED: Use let instead of const so session can be updated
// Also call ensureSessionSync to make sure we have the latest session ID
let currentSessionId = sessionStorage.getItem('esg_session_id');

// Helper to get the current session ID (always fresh from storage)
function getCurrentSessionId() {
    // First try to sync from data-layer
    if (window.esg && window.esg.ensureSessionSync) {
        window.esg.ensureSessionSync();
    }
    // Then get from sessionStorage (may have been updated by sync)
    currentSessionId = sessionStorage.getItem('esg_session_id');
    return currentSessionId;
}

// DEBUG: Log initial session state
console.log(`[WHITE CELL INIT] Session ID at load: ${currentSessionId}`);
console.log(`[WHITE CELL INIT] hasUtil (Supabase): ${hasUtil}`);
...
function getWhiteCellKey(move = currentMove) {
    // FIXED: Use session-based key when session is available, with legacy fallback
    const sessionId = currentSessionId || sessionStorage.getItem('esg_session_id');
    if (sessionId) {
        return `whiteCell_session_${sessionId}_move_${move}`;
    }
...
function getFacilitatorSubmissionKey(move = currentMove) {
    ...
    const sessionId = currentSessionId || sessionStorage.getItem('esg_session_id');
    if (sessionId) {
        return `blueActions_session_${sessionId}_move_${move}`;
    }
...
function getRequestsKey(move = currentMove) {
    ...
    const sessionId = currentSessionId || sessionStorage.getItem('esg_session_id');
    if (sessionId) {
        return `blueRequests_session_${sessionId}_move_${move}`;
    }
    return (window.buildMoveKey && buildMoveKey(move, 'requests')) || `blueRequests_move_${move}`;
}

// NEW: Helper to get communications key with session support
function getCommunicationsKey(move = currentMove) {
    const sessionId = currentSessionId || sessionStorage.getItem('esg_session_id');
    if (sessionId) {
        return `communications_session_${sessionId}_move_${move}`;
    }
    return `communications_move_${move}`;
}

// NEW: Helper to get feedback key with session support
function getFeedbackKey(move = currentMove) {
    const sessionId = currentSessionId || sessionStorage.getItem('esg_session_id');
    if (sessionId) {
        return `whiteCellFeedback_session_${sessionId}_move_${move}`;
    }
    return `whiteCellFeedback_move_${move}`;
}

// NEW: Helper to check multiple key patterns for data (session-based first, then legacy)
function getDataFromMultipleKeys(keyPatterns, defaultValue = null) {
    for (const key of keyPatterns) {
        const data = safeGetItem(key, null);
        if (data !== null) {
            console.log(`[DEBUG] Found data in key: ${key}`);
            return { data, key };
        }
    }
    console.log(`[DEBUG] No data found in keys:`, keyPatterns);
    return { data: defaultValue, key: null };
}
...
async function loadSubmittedRequests() {
    ...
    // FIXED: Use helper to get fresh session ID
    const activeSessionId = getCurrentSessionId() || (window.esg && window.esg.getCurrentSessionId ? window.esg.getCurrentSessionId() : null);

    console.log(`[WHITE CELL DEBUG] loadSubmittedRequests called`);
    console.log(`[WHITE CELL DEBUG] - activeSessionId: ${activeSessionId}`);
    console.log(`[WHITE CELL DEBUG] - currentMove: ${currentMove}`);

    if (hasUtil && activeSessionId) {
        try {
            const dbRequests = await window.esg.fetchRequests(activeSessionId);
            console.log(`[WHITE CELL DEBUG] - Database fetch result: ${dbRequests ? dbRequests.length : 'null'} requests`);
            if (dbRequests && Array.isArray(dbRequests)) {
                requests = dbRequests.filter(r => r.phase === currentPhase || !r.phase);
            }
        } catch (error) {
            console.error('[WHITE CELL DEBUG] Error loading requests from database:', error);
            // Fall through to localStorage fallback
        }
    }
    ...
    if (requests.length === 0) {
        console.log(`[WHITE CELL DEBUG] No database requests, checking localStorage...`);

        // FIXED: Try multiple key patterns in order of preference
        const keyPatterns = [
            // Session-based keys (preferred)
            `blueRequests_session_${activeSessionId}_move_${currentMove}`,
            // Current session key via helper
            getRequestsKey(),
            // Legacy keys
            `blueRequestsSubmittedMove${currentMove}`,
            `blueRequests_move_${currentMove}`
        ].filter(Boolean);

        console.log(`[WHITE CELL DEBUG] Checking localStorage keys:`, keyPatterns);
        ...
        for (const key of keyPatterns) {
            ...
            if (data) {
                requestsData = data;
                foundKey = key;
                console.log(`[WHITE CELL DEBUG] Found data in key: ${key}`, data);
                break;
            }
        }
        ...
        if (requestsData) {
            try {
                ...
                console.log(`[WHITE CELL DEBUG] Extracted ${requests.length} requests from ${foundKey}`);
            } catch (e) {
                console.error('[WHITE CELL DEBUG] Error parsing requests data:', e);
                container.innerHTML = `
            ...
            console.log(`[WHITE CELL DEBUG] No localStorage data found in any key pattern`);
        }
    }
...
async function loadSubmittedActions() {
    ...
    console.log(`[WHITE CELL DEBUG] loadSubmittedActions called`);
    console.log(`[WHITE CELL DEBUG] - activeSessionId: ${activeSessionId}`);
    console.log(`[WHITE CELL DEBUG] - currentMove: ${currentMove}`);
    console.log(`[WHITE CELL DEBUG] - hasUtil (Supabase): ${hasUtil}`);

    if (hasUtil && activeSessionId) {
        try {
            const dbActions = await window.esg.fetchActions(currentMove, activeSessionId);
            console.log(`[WHITE CELL DEBUG] - Database fetch result: ${dbActions ? dbActions.length : 'null'} actions`);
            if (dbActions && Array.isArray(dbActions)) {
                actions = dbActions;
            }
        } catch (error) {
            console.error('[WHITE CELL DEBUG] Error loading actions from database:', error);
            // Fall through to localStorage fallback
        }
    }
    ...
    // Fallback to localStorage if database not available or empty
    if (actions.length === 0) {
        console.log(`[WHITE CELL DEBUG] No database actions, checking localStorage...`);

        // FIXED: Try multiple key patterns in order of preference
        const keyPatterns = [
            // Session-based keys (preferred)
            `blueActions_session_${activeSessionId}_move_${currentMove}`,
            // Legacy unified keys
            getFacilitatorSubmissionKey(),
            (window.buildMoveKey && buildMoveKey(currentMove, 'facilitator')) || `actions_move_${currentMove}`,
            // Very old legacy keys
            `blueActionsSubmittedMove${currentMove}`,
            `blueActions_move_${currentMove}`
        ...
        if (submission) {
            ...
            if (Array.isArray(submission)) {
                actions = submission;
            } else if (submission.actions && Array.isArray(submission.actions)) {
                actions = submission.actions;
            } else if (typeof submission === 'object') {
                // Try to extract actions from object
                actions = Object.values(submission).filter(v => v && typeof v === 'object' && v.mechanism);
            }
            console.log(`[WHITE CELL DEBUG] Extracted ${actions.length} actions from ${foundKey}`);
        } else {
            console.log(`[WHITE CELL DEBUG] No localStorage data found in any key pattern`);
        }
    }
