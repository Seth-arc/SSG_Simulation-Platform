...
async function addAction() {
    ...
    // DEBUG: Log session state before submitting
    console.log(`[FACILITATOR DEBUG] addAction called`);
    console.log(`[FACILITATOR DEBUG] - hasUtil: ${hasUtil}`);
    console.log(`[FACILITATOR DEBUG] - currentSessionId: ${currentSessionId}`);
    console.log(`[FACILITATOR DEBUG] - sessionStorage esg_session_id: ${sessionStorage.getItem('esg_session_id')}`);

    // Ensure session is synced before submitting
    if (window.esg && window.esg.ensureSessionSync) {
        const syncedId = window.esg.ensureSessionSync();
        console.log(`[FACILITATOR DEBUG] - After sync, session ID: ${syncedId}`);
    }
    ...
    if (hasUtil && currentSessionId) {
        try {
            ...
            console.log(`[FACILITATOR DEBUG] Submitting action with formData:`, formData);
